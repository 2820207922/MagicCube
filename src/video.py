...
class Webcam:
    ...
    def __init__(self):
        ...
        self.colors_to_calibrate = ['white', 'red', 'green',
                                    'orange', 'blue', 'yellow']
    ...
    def draw_preview_stickers(self):
        ...
        self.draw_stickers(self.preview_state,
                           STICKER_AREA_OFFSET, STICKER_AREA_OFFSET)
        ...
        for index, (x, y, w, h) in enumerate(contours):
            if index in self.average_sticker_colors and len(self.average_sticker_colors[index]) == max_average_rounds:
                ...
                most_common_color = max(
                    sorted_items, key=lambda i: sorted_items[i])
            ...
            closest_color = color_detector.get_closest_color(avg_bgr)[
                'color_bgr']
    ...
    def update_snapshot_state(self):
        ...
        center_color_name = color_detector.get_closest_color(self.snapshot_state[4])[
            'color_name']
    ...
    def draw_current_color_to_calibrate(self):
        ...
        if self.done_calibrating:
            ...
            for index, text in enumerate(messages):
                ...
                self.render_text(text, (int(self.width / 2), y),
                                 size=font_size, anchor='mt')
        else:
            ...
            self.render_text(text, (int(self.width / 2), offset_y),
                             size=font_size, anchor='mt')
    ...
    def draw_calibrated_colors(self):
        ...
        for index, (color_name, color_bgr) in enumerate(self.calibrated_colors.items()):
            ...
            self.render_text(i18n.t(color_name), (20, y1 +
                             STICKER_AREA_TILE_SIZE / 2 - 3), anchor='lm')
    ...
    def draw_2d_cube_state(self):
        ...
        offset_x = self.width - (side_size * 4) - \
            (side_offset * 3) - MINI_STICKER_AREA_OFFSET
        offset_y = self.height - (side_size * 3) - \
            (side_offset * 2) - MINI_STICKER_AREA_OFFSET
    ...
    def get_result_notation(self):
        ...
        for side, preview in notation.items():
            for sticker_index, bgr in enumerate(preview):
                notation[side][sticker_index] = color_detector.convert_bgr_to_notation(
                    bgr)

        # Join all the sides together into one single string.
        # Order must be URFDLB ('red', 'green', 'white', 'orange', 'blue', 'yellow')
        combined = ''
        for side in ['red', 'green', 'white', 'orange', 'blue', 'yellow']:
    ...
    def run(self):
        ...
        while True:
            ...
            if key == 13:
                transform.transform_2D_to_3D(self.result_state)
                self.result_state = transform.tranform_3D_to_2D()
            ...
            if len(contours) == 9:
                ...
                if not self.calibrate_mode:
                    ...
                    self.done_calibrating = self.current_color_to_calibrate_index == len(
                        self.colors_to_calibrate)
                    if self.done_calibrating:
                        color_detector.set_cube_color_pallete(
                            self.calibrated_colors)
                        config.set_setting(
                            CUBE_PALETTE, color_detector.cube_color_palette)
